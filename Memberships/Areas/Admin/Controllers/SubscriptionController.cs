...
namespace Memberships.Areas.Admin.Controllers
{
    public class SubscriptionController : Controller
    {
        ...
        private async Task<SubscriptionProduct> GetProductItem(int? subscriptionId, int? productId)
        {
            try
            {
                int subId = 0, prdId = 0;
                ...
                var subscriptionProduct = await db.SubscriptionProducts.FirstOrDefaultAsync(
                    pi => pi.ProductId.Equals(prdId) && pi.SubscriptionId.Equals(subscriptionId));
